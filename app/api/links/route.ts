...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      logger.warn("KV cache set failed after link creation", { slug, error: kvErr });
    }

    logger.info("Link created", { slug, userId: session.user.id });
    return NextResponse.json(link, { status: 201 });
  } catch (err: any) {
    logger.error("POST /api/links failed", { error: err.message || err });
    console.error("Link creation raw error:", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
